...
export default function DealerDashboard() {
  ...
  return (
    <section className="space-y-4">
      ...
      {listings.length===0?<p className="text-zinc-300">No listings yet.</p>:listings.map(l=><div key={l.id} className="border border-white/10 bg-white/5 rounded-2xl p-4">{l.title}</div>)}
    </section>
  );
}
